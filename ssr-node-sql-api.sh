...
Github="https://github.com/iboyan/ssr.git"
...
api_new(){
	...
	read -p "是否开启后端TrueIP(回车默认不开启 Y/N):  " TRUEIP
	...
	TRUEIP=${TRUEIP:-"N"} #默认不开启TRUEIP
	if [[ ${TRUEIP} == "Y" || ${TRUEIP} == "y" ]];then
	sed -i '/TRUE_IP/c \TRUE_IP = True' ${config}
	else
	sed -i '/TRUE_IP/c \TRUE_IP = False' ${config}
	fi
...
api_old(){
	...
	TRUEIP=${TRUEIP:-"N"} #默认不开启TRUEIP
	if [[ ${TRUEIP} == "Y" || ${TRUEIP} == "y" ]];then
	sed -i '/TRUE_IP/c \TRUE_IP = True' "/root/${NODE_LIST}/userapiconfig.py"
	else
	sed -i '/TRUE_IP/c \TRUE_IP = False' "/root/${NODE_LIST}/userapiconfig.py"
	fi
	#替换守护程序
