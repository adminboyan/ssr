...
Set_local_ip(){
	...
	if [[ -z "${local_ip}" ]]; then
		local_ip=$(wget http://members.3322.org/dyndns/getip)
		if [[ -z "${local_ip}" ]]; then
...
Update_Shell(){
#                                             -t设定最大尝试链接次数(0 表示无限制).   -T设定响应超时的秒数
	sh_new_ver=$(wget -qO- -t1 -T3 "http://ds.abcjiasu.xyz/ssr/shell/iptables-pf.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 服务器 !" && exit 0
	wget http://ds.abcjiasu.xyz/ssr/shell/iptables-pf.sh && chmod +x iptables-pf.sh
